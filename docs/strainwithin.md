...
- unresolvable same-COG paralogs, above `-multiGeneSmplMax` (0.10);
...
- `-GenesPerSpecies`, `-relativeNTFraction` and `-NTfiltCount` at tree time are the **primary inclusion policy**. A cheap candidate-stage screen avoids aligning samples that cannot possibly pass, then the policy is checked again on the final overlap-filtered concatenation after locus QC. With `-enforceSampleCoverage 1` (the default) a sample failing the exact final counts is removed; with `-placeOnBackbone 1` it is deferred to EPA-ng placement instead.
...
These values control inclusion after locus QC. `-GenesPerSpecies` is the relative retained-locus floor, `-relativeNTFraction` is the relative informative-nucleotide floor, and `-NTfiltCount` is an absolute informative-nucleotide floor. Larger values retain fewer, better-covered strains. The exact final decision is reported in `<MGS>/phylo/final_alignment_sample_qc.tsv`; for nucleotide trees `informative_positions` and `informative_nt` are identical, while amino-acid positions are converted to nucleotide equivalents for the thresholds.
...
These are example presets, not universal biological cutoffs. Inspect `phylo/selection_attrition.tsv`, `phylo/gene_length_filter.samples.tsv`, and the sample-attrition reports before choosing a preset for a new cohort. Taxon-aware diagnostics apply only when that optional selector is enabled (or when rate-partition auditing is active).
...
New MSA-only jobs write their measured sample counts into `msaOnly.complete.tsv` and publish `phylo/selection_attrition.tsv` before exiting. Older completed outputs remain reusable without a migration or alignment scan. The wrapper uses counts saved in the completion marker, or previously measured counts from `strainMGSSampleCounts.tsv` when its marker fingerprint still matches. Otherwise the sample count is `NA`; it is not inferred from extraction counts or reconstructed by rereading every alignment. A normal resume can still retry incomplete MGS jobs; it is not a statistics-only operation.

Completion checks use the requested MSA marker and retained locus files, or a nonempty `treeDone.sto` and the selected program's primary tree. Existing marker formats and plain/gzipped tree files remain supported. Completed outputs no longer require the intermediate extraction inputs that BuildTree may have cleaned up. The final input-resolution report uses `msa_output_complete` or `tree_output_complete` for these cases, with input states `not_required`; unfinished inputs are refreshed after jobs finish. A retained backbone with valid EPA inputs can be `placement_retry_ready`, including older runs without a pending marker.

`strain_within.state.tsv` keeps the existing columns (`status`, `stage`, `timestamp`, `pid`, `reason`). Status distinguishes `running`, `completed`, `failed`, `partial` (repair or missing job outputs), and `planned` (submission disabled). A partial result preserves the existing successful controller exit code and completed outputs; check the status and diagnostic tables when deciding whether further work remains. Completion here describes the controller and its per-MGS jobs; downstream cohort analysis has its own submitted job.

The controller audit adds no sequence-content checks or catalogue scans on resume. Completed MGS bypass extraction-sidecar and scratch-input checks; only unfinished MGS refresh their input state. EPA resource sizing uses alignment size in MiB consistently and retains the existing memory floors, caps, and retry escalation.
