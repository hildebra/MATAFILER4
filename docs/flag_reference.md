...
Phase II does not place catalogue-wide outgroup discovery in front of all tree submissions. It prepares a small first reference batch of 32 actionable MGS so the first tree jobs are submitted promptly, then uses batches of 256 to amortize gene-map and FASTA-index scans while earlier trees run. Within each batch, the controller embeds the selected outgroup in the staged overlay and immediately queues that MGS tree; rebuilding the selective reference cache for a later batch cannot affect already staged or submitted jobs. Batch boundaries and the gene-map, candidate-index, and exact-reference-ID stages are reported explicitly.
