...
| `buildTree5.pl` | `5.36` | Phylogenetic tree construction and related MSA/population-genetic analyses. |
...
Phylogenetic tree construction and related MSA/population-genetic analyses. The source reports version `5.36`.
...
In strict-backbone IQ-TREE mode, `IQtree_allsites.backbone.treefile` contains the ML-inferred backbone and `IQtree_allsites.treefile` is the default primary tree containing the nearest-backbone grafts. Each deferred query is grafted beside its closest backbone sample by observed distance over mutually called sites. This is not likelihood-based phylogenetic placement and does not refine the inferred backbone topology. For taxon-aware strain trees, the broad backbone still retains every non-severe coverage sample, but a deferred sample must pass `-GenesPerSpecies` times final selected-locus Q90 (with a floor of two loci) and `-NTfilt` times final informative-NT Q90, as well as `-NTfiltCount` and `-placementMinOverlap`, before it can be grafted. `phylo/taxon_aware_placement_eligibility.tsv` and `phylo/strict_backbone.samples.tsv` record the decision and exclusion reason.
