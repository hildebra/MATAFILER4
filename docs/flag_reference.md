...
| `buildTree5.pl` | `5.88` | Phylogenetic tree construction and related MSA/population-genetic analyses. |
...
| `-jobNice` | integer | `2500` | advanced | Slurm `--nice` handicap on ordinary tree and Phase-I jobs; OOM retries always submit at `--nice=0` so they outrank a backlog this run already queued. Rescanning alone cannot reorder jobs Slurm has already accepted. `0` disables it. |
...
| `-multiGeneSmplMax` | float | `0.10` | stable | Maximum ambiguous/multigene-locus fraction per sample. |
...
| Aliases | Type | Default | Status | Description |
|---|---:|---|---|---|
| `-GeneLengthMin` | float | `0.4` | stable | Minimum fraction of the locus length-Q90 used for sample/locus QC. |
| `-GeneLengthIncludeMin` | float | `-GeneLengthMin` (`0.4`) | stable | MSA inclusion gate. Unless explicitly supplied, it inherits the final `-GeneLengthMin` value. A lower explicit value enables post-QC partial-observation recovery and cannot exceed `-GeneLengthMin`. |
...
| `-NTfiltCount` | integer | `5000` | stable | Absolute backbone floor on informative NT in the final overlap-filtered concatenation after locus QC; roughly five well-covered loci. `0` leaves only the relative gates. |
...
| `-taxonAwareLocusSelection` | integer | `0` | stable | Opt in to robust-core plus taxon-rescue locus selection after MSA QC. The default retains buildTree5's hard 30%-of-Q90 locus prevalence filter. |
...
| `-postAlignmentSequenceOutlierMask` | integer | `1` | stable | Remove isolated, strongly divergent ingroup records from individual loci in the existing MSAfix 2.16 QC pass. Configured outgroups are exempt. Set to `0` to disable. |
...
| `-NTfiltPerGene` | float | `0.4` | stable | Per-locus minimum informative-NT fraction; sequences below it are dropped from that locus. |
| `-GeneLengthIncludeMin` | float | `-NTfiltPerGene` (`0.4`) | stable | Per-sample locus-length-Q90 fraction admitted to MSA. Unless explicitly supplied, it inherits the final `-NTfiltPerGene` value; a lower explicit value enables post-QC recovery, whose observations do not count toward backbone/placement eligibility. |
...
| `-fracMaxGenes90pct` | float | `0.3` | stable | Hard minimum locus size as a fraction of the category-size Q90 when taxon-aware selection is disabled. Set to `0` to retain every category containing at least one length-filtered sequence. |
| `-NTfiltCount` | integer | `0` | stable | Backbone absolute minimum informative nucleotide count in the final overlap-filtered concatenation after locus QC. |
...
| `-ensureLocusMSAs` | integer | `0` | advanced | Backfill the per-locus `MSA/*.fna.gz` checkpoints beside an already completed tree. Honoured whenever a validated tree is present and no parameter that determines the per-locus alignments has changed; the tree method, downstream analyses, concatenation partitioning, post-alignment locus thresholds and scheduler allowances are all ignored, because such a run exits before reaching them. The loci are then realigned, `msaOnly.complete.tsv` is written, and the tree, its completion marker, the concatenated alignment and the recorded tree-stage policy are left untouched. Such a job never rebuilds a phylogeny: if it cannot recover, it exits with an error naming the blockers and changes nothing. Set by `strain_within.pl` under `-rmMSA 0`; incompatible with `-onlyMSA 1`. |
...
| `-enforceSampleCoverage` | integer | `0` | stable | Apply `-GenesPerSpecies`, `-relativeNTFraction` and `-NTfiltCount` as a sample-removal policy. A cheap candidate screen remains in the ordinary path; all multi-locus trees remeasure the thresholds on the final overlap-filtered concatenation after locus QC. Inactive under `-placeOnBackbone 1`, where failing samples are routed to placement instead. Off by default so callers that set no inclusion policy keep their previous behaviour. The exact final counts are in `phylo/final_alignment_sample_qc.tsv`. |
...
| `-postAlignmentSequenceOutlierMask` | integer | `0` between species; `1` within species | stable | Use MSAfix 2.16 during locus QC to remove isolated, strongly divergent non-outgroup sequence records from individual MSAs. Requires `-postAlignmentLocusQC 1`. The native safeguards require sufficient comparable sites and cohort size, both an absolute divergence floor and modified-Z support, and no more than 10% outlier candidates; larger divergent groups are retained. Writes `phylo/post_alignment_sequence_outliers.tsv`. |
...
| `-taxonAwareLocusSelection` | integer | `0` | stable | Opt in to two-stage locus selection. When disabled, the hard `-fracMaxGenes90pct` prevalence filter is used. When enabled, a permissive robust/core-plus-rescue candidate set is aligned first; final ranking uses robust support, high-confidence occupancy/prevalence and the optional presorter. |
...
| `-taxonAwareHeterogeneityScoring` | integer | `1` | advanced | Include variable-site information credit and excess-variation penalties in final locus ranking when taxon-aware selection is enabled. Set to `0` for a prevalence/completeness-only taxon-aware score. |
...
Broad or between-species phylogeny is the default. In this mode `buildTree5.pl` does not remove columns using a fixed taxon-count overlap threshold or mask within-locus sequence outliers, and post-alignment QC checks locus structure/occupancy without rejecting deep AA divergence. Use `-withinSpecies 1` for strain or other within-species trees. `-minOverlapMSA`, `-postAlignmentDivergenceQC`, and `-postAlignmentSequenceOutlierMask` can override the individual mode defaults. Existing `-strainWithinPreset 1` calls remain compatible and imply within-species mode. With `-continue 1`, the stored QC policy prevents reuse of a concatenated alignment built under different masking settings.
...
Taxon-aware selection is optional (`-taxonAwareLocusSelection 1`) and stays inside `buildTree5.pl` so it can reuse the normal alignment and MSAfix stages. With the default `0`, BuildTree instead applies the hard `-fracMaxGenes90pct 0.3` locus-prevalence filter. When opted in, the pre-MSA pass ranks length-stable, complete, prevalent loci and retains a 400-locus robust core. If `-preferredCoreGenes` is supplied, a locus whose catalogue seed is listed in that GTDB/core guide is ranked ahead of other eligible loci and receives a modest secondary preference during greedy taxon rescue; it still must pass normal sequence and QC filters, and it does not relax the hard broad-availability gate for taxon rescue or QC backfill. Greedy taxon rescue and backfill remain restricted to loci found in at least 80% of taxa with any usable candidate locus. Heterogeneity-dependent information rewards and excess-variation penalties are active in this optional selector by default; pass `-taxonAwareHeterogeneityScoring 0` to omit them. After MSAfix, the final pass recalculates prevalence and reapplies the same guard. Candidate and final rows expose `preferred_core`, `coverage_rescue_eligible`, and `coverage_rescue_reason`. By default, final taxon-aware and rate-partition reports are sectioned into `phylo/taxon_aware_diagnostics.tsv`; set `-compactTaxonAwareDiagnostics 0` for the individual files.

`strain_within.pl` also defaults taxon-aware selection off, so its ordinary path uses the hard 30%-of-Q90 locus-prevalence filter; pass `-taxonAwareLocusSelection 1` to opt in to robust-core plus taxon-rescue selection. In that optional mode it forwards the 0.8 rescue-prevalence guard and automatically uses the supplied `-MGS` guide when it ends in `.core` (otherwise an available sibling `-MGS.core`) as `-preferredCoreGenes`. Its filtering defaults are one `0.4` gate for both `-GeneLengthMin` QC and `-GeneLengthIncludeMin` MSA inclusion, ordinary-tree `-GenesPerSpecies 0.2`, `-relativeNTFraction 0.1`, and a 5,000-informative-NT absolute sample floor. Supplying a lower `-GeneLengthIncludeMin` explicitly restores post-QC partial-observation recovery. Backbone placement remains disabled unless `-placeOnBackbone 1` is supplied; only then do the `0.04` locus, `0.03` NT, and 10,000-site shared-backbone placement thresholds apply. Within-species locus QC rejects cross-locus consensus-divergence outliers above modified-Z 5.0. The strain preset uses the legacy IQ-TREE command by default; pass `-iqLegacy 0` to use the standard modern command, or `-iqPathogen 1` to select the IQ-TREE 3 pathogen path.
