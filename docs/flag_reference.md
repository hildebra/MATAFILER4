...
Phase II reuses the Phase I selected-gene map and reads the relevant candidate-outgroup FNA and FAA records with the normal sequential FASTA reader. It creates neither a selective-reference cache nor `.fai` files, avoiding the random-access and cache-build delay. Once the single direct reference pass is complete, each MGS gets its selected outgroup overlay and is queued as an individual tree job in the normal order.
