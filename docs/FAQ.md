...
### The run did not submit

1. Check the `.mfc` stdout and stderr files.
2. Check `config.txt`, especially `MFLRDir`, `DBDir`, `globalTmpDir` and `nodeTmpDir`.
3. Run the exact command with `-submit 0`.
4. Check that the mapping file is tab-delimited and uses Linux line endings.

### Jobs submitted but failed

1. Inspect the relevant sample-level `LOGandSUB/` directory.
2. Read both `.otxt` and `.etxt` files.
3. Check whether failure is node-specific, memory-specific or path-specific.
4. If the local node temporary directory is not writable, exclude the affected node with `-excludeNodes` and inform the local cluster administrator.

### Jobs are stuck in dependency states

1. Inspect dependencies with `scontrol show job <jobid>` or the equivalent for your scheduler.
2. Remove jobs that can never run, if this matches local scheduler policy.
3. Rerun the same MATAFILER4 command after the queue is clean.
4. On Slurm, `-killDepNever 1` can be used to remove jobs in dependency-never-met states.

### Expected outputs are missing

1. Wait until all submitted jobs have finished or failed.
2. Rerun the same command; MATAFILER4 should continue missing steps.
...
**Solution:** Adding 2 files into your MFF micromamba environment will help you resolve the issue (contact your local MATAFILER4 administrator).
...
**Solution:** We have an associated repository named [MAGRec-TK](https://github.com/huminfo8/magrec-tk/tree/main) which you can install and run on the MATAFILER4 output. 
