...
like($between_source,
	qr/post_alignment_locus_qc\.policy\.tsv.*?\$policy\{schema\}.*?eq "3".*?\$policy\{enabled\}.*?eq "0".*?\$policy\{scope\}.*?eq "between".*?\$policy\{per_gene_length_fraction\}.*?eq "0\.4".*?\$policy\{minimum_category_q90_fraction\}.*?eq "0".*?\$policy\{minimum_gene_fraction_per_species\}.*?eq "0\.3".*?if \(!-s \$treeFile \|\| !\$broadLocusRetentionCurrent\)/s,
	'between-MGS phylogeny rebuilds empty or differently filtered tree checkpoints');
...
like($between_source,
	qr/-bootstrap 1000.*?-NTfilt 0\.5.*?-NTfiltPerGene 0\.4.*?-GenesPerSpecies 0\.3.*?-fracMaxGenes90pct 0.*?-postAlignmentLocusQC 0.*?-MSAprogram \$MSAprog.*?-AutoModel 1/s,
	'between-MGS trees retain rare nonempty loci while using relaxed broad species filtering and partition-aware model selection');
