...
use Mods::GenoMetaAss qw( systemW readFasta readFastHD writeFasta convertMSA2NXS);
use Mods::phyloTools qw(getTreeLeafs calcDisPos2 runRaxML runRaxMLng runQItree runFasttree fixHDs4Phylo);
...
my $ntCntTotal =0; my $bootStrap=0;
my ($fnFna, $aaFna,$cogCats,$outD,$ncore,$Ete, $filt,$smplDef,$smplSep,$calcSyn,$calcNonSyn,
			$useAA4tree,$calcDNAdiff,$tmpD ) = ("","","","",12,0,0.8,1,"_",0,0,0,0,"");

my ($continue,$isAligned) = (0,0);#overwrite already existing files?
my $outgroup="";
my $fixHeaders = 0;
my ($doGubbins,$doCFML,$doRAXML,$doFastTree, $doIQTree,$doRAXMLng) = (0,0,0,1, 0, 0);#fastree as default tree builder
...
######
$stBin = getProgPaths("supertree") if ($doSuperTree);
...
$doMSA =0 if ($isAligned || (
			$continue && -e $multAli && (-e $multAliSyn ||!$calcSyn)&& (-e $multAliNonSyn ||!$calcNonSyn)) );  ## checks if MSA already exists
#die "$doMSA $continue\n";
#die "$doDNDS\n";
...
if ($isAligned){
	...
	foreach (@linesCats){ #check first some parameters..
		...
		foreach my $seq (@spl){
			...
			#filter per sequence 
			my $num1 = $ss =~ tr/[\-Xx]//;
			$charCnts{$sp}{$seq} = (length( $FAA{$seq})-$num1);
	...
	my $qtl90NTcntAll =  (sort { $a <=> $b }( @allNTcnts)) [ int($#allNTcnts*0.9) ];

	my %smplsRmvd; my $tooFewGenes=0;my $tooFewNTs=0;my $tooFewNTs2=0;
	print "Samples removed due to low gene presence:\n";
	foreach my $sp (keys %specList){
		my $NTfilter = $totalNTs{$sp} < $qtl90NTcntAll * $ntFrac;
		my $factor = 3 ; #AA counts..
		
		my $NTfilter2 = ( $totalNTs{$sp} <  ($ntCntTotal*$factor) );

		if ($specList{$sp} <  $maxGenes * $GeneFracPSpec  || $NTfilter || $NTfilter2){
			$smplsRmvd{$sp}=1;
			if ($NTfilter){
				$tooFewNTs++;
			} elsif ($NTfilter2){
	...
	#die "$maxGenes\n";
	print "\n\n---Prefilter---\nTotal Smpls: ".scalar(keys%specList).", MaxGenes: $maxGenes, MaxAA: $maxNtCntTotal,$qtl90NTcntAll\n";
	print "Smpls removed: <AAs($ntFrac,$ntCntTotal):$tooFewNTs,$tooFewNTs2 ; <genes($GeneFracPSpec):$tooFewGenes\n";
	print "Remaining Smpls: ".(scalar(keys%specList) - $tooFewNTs - $tooFewNTs2 - $tooFewGenes) . "\n";
	...
	foreach (@linesCats){
		...
		foreach my $seq (@spl){### $seq = genomeX_NOGY
			...
			if ($specList{$sp} <  $maxGenes * $GeneFracPSpec){die "buildTree: GeneFracPSpec maxGenes shouldn't be here!\n";}
			my $seq2 = $seq;
...
#			if ($calcDistMatExt && -e $inFastaOth){
		...
		if ( $doGenesToPh){
			...
			system "rm -f $phylipD/$spl2[1].$cnt.fna.ph*\n";
			my $cmd2 = "$fasta2phylip -c 50 $MSAfn > $phylipD/$spl2[1].$cnt.fna.ph\n";
			#die "$cmd2\n";
	...
	#even in supertree case, needs the merged MSA to create tree distances
	if ($useAA4tree){
	...
	my ($multAli,$r1,$r2) = singleGeneMSAprocess($multAli,\@MSAs,\@MSA_AA);
	@MSAs = @{$r1};	@MSA_AA = @{$r2};
}
#die;
...
if ($doSuperTree || $doSuperCheck){#can be for 2 reasons: 1) build actual super tree 2) quality control
	...
	for (my $i=0;$i<@theRealMSAs;$i++){
		print "===============>  Subtree $i  <===============\n";
		my $trRetH = treeAtHeart($theRealMSAs[$i],"allsites",$i,1,"");
...
#-------------------------------------------
my $Hr = treeAtHeart($multAli,"allsites","",0,"");
my %Tree1 = %{$Hr};
if ($calcSyn){ #tree at syn pos
	treeAtHeart($multAliSyn,"syn","",0,$Tree1{nwk});
} 
if ($calcNonSyn){ #tree at non-syn pos
	treeAtHeart($multAliNonSyn,"nonsyn","",0,$Tree1{nwk});
}
...
sub treeAtHeart{
	...
	if ($consTree ne ""){
		...
		if ($unpresent){
			...
			system "rm $multF;mv $multF.tmp $multF";
		} else {
			@genomeList = keys %FNA;
		}
	...
	if ($doPhym){
		...
		my @thrs;
		my $tcmd = "";
		my $nwkFile = "$treeD/phyml${tcnt}_${siteTag}.nwk";
...
#		my @spl2 = split /$smplSep/,$Mkeys[0];
		...
		if ($len == 0 || !defined($MFAA{$Mkeys[0]})){print STDERR "0 length sequence discovered\n";next ;}
		push(@lengthsParts,$len);
		#print $len.$MFAA{$Mkeys[0]}."\n";
	...
	foreach my $kk (@ksMSAFAA){
		...
		if ( $charCnts{$kk} > $maxNtCnt){$maxNtCnt = $charCnts{$kk};}
	...
	foreach my $kk (@ksMSAFAA){
		my $num1 = $charCnts{$kk};
		#print "$num1\n";
		if ( ( ($num1 / $maxNtCnt ) < $ntFrac) || ($num1 < ($ntCntTotal/$factor) ) ){
	...
	for (my $i=0;$i<@lengthsParts;$i++){
		#DNA, part1 = 1-100
		print O "DNA, part".($i+1) ." = ". ($lastP+1) ."-". ($lengthsParts[$i]+$lastP) ."\n";
...
sub pogenStatsFilter{
	...
	system "mkdir -p $codemlOutD" unless (-d $codemlOutD);
	my $cmd = "$RpogenS $outD $mapF $codemlOutD $MSAsubsD $subsetPopgenStats";
	#die "$cmd\n";
	my $ret = `$cmd`;
	$ret =~ m/Outliers: (.*);;/;
	my @spl = split /,/,$ret;
...
sub selecAnalysis($ $ $ $ $){
	...
	system "rm -r $codemlOutDTmp" if (-e $codemlOutDTmp);
