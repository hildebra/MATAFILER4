...
sub executable {
	...
	if (defined($override) && $override ne '') {
		die "Configured executable is not executable: $override\n" unless -x $override;
		return $override;
	}
	for my $name (@names) {
...
my $wget;
sub wget_tool {
	$wget ||= executable($opt{wget}, 'wget');
	die "wget is required to query archive metadata and download ENA files\n"
		unless defined $wget;
	return $wget;
}
...
sub download_sra {
	my $prefetch = executable($opt{prefetch}, 'prefetch')
		or die "NCBI SRA downloads require the SRA Toolkit executable 'prefetch'\n";
	my $fasterq = executable($opt{fasterq_dump}, 'fasterq-dump')
		or die "NCBI SRA downloads require the SRA Toolkit executable 'fasterq-dump'\n";
	my $validator = executable($opt{vdb_validate}, 'vdb-validate')
		or die "NCBI SRA downloads require the SRA Toolkit executable 'vdb-validate'\n";
	my $compressor = executable($opt{compressor}, 'pigz', 'gzip')
		or die "SRA FASTQ conversion requires pigz or gzip\n";
