...
$SIG{HUP} = sub { die "MGS controller received SIGHUP\n" };
_mgs_workflow_stage('preflight');
preflight_directory($outD, 'MGS output directory');
...
retry_unlink($iniMB2sto, label => 'invalidate stale MAG-quality checkpoint')
